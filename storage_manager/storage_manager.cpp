...
PATCH_DATA_X StorageManager::read_patch(uint8_t bank, uint8_t patch)
{
    ...
    location+= PATCH_MIDI_PC_SIZE; // no need to copy as nothing to read after program change messages
...
uint8_t StorageManager::write_patch_title(uint8_t bank, uint8_t patch, uint8_t* title)
{
    ...
    eeprom.write_multiple_bytes(title, byte_address, CAT24C32_PAGE_SIZE);

    // for(int i = 0; i < CAT24C32_PAGE_SIZE; i++)
    // {
    //     eeprom.write_byte(title[0], (byte_address + i));
    // }

    eeprom.read_multiple_bytes(byte_address, CAT24C32_PAGE_SIZE, read_buffer);
    result = memcmp(read_buffer, write_buffer, CAT24C32_PAGE_SIZE);
    return result;
}
    ...
    if(header_not_found || footer_not_found)
    {
        ...
        for(int bank = 0; bank < NUM_BANKS; bank++)
        {
            for(int patch = 0; patch < NUM_PATCHES; patch++)
            {
                ...
                memset(read_buffer, 0, CAT24C32_PAGE_SIZE);
                eeprom.read_multiple_bytes(PATCH_DATA_OFFSET + (bank * BANK_DATA_SIZE) + (patch * PATCH_DATA_SIZE), CAT24C32_PAGE_SIZE, read_buffer);

                memset(string_buffer, 0, sizeof(string_buffer));
                memcpy(string_buffer, read_buffer, CAT24C32_PAGE_SIZE);
